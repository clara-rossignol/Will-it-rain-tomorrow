...
function generate(data; option = "drop", valid = "true", test = "true")
    ...
    if option == "drop"
        ...
        coerce!(new_data, Count => Continuous);
        ...
        if valid == "true"
            if test == "true"
                ...
                "We do not provide validation set without a test set, look at your arg."
                empty = DataFrame()
                empty
            end
        elseif valid == "false"
    ...
    elseif option == "med"
        ...
        new_data = insertcols!(new_data,:precipitation_nextday=>data.precipitation_nextday[:]);
        coerce!(new_data, Count => Continuous);
        coerce!(new_data, :precipitation_nextday => Multiclass);
        idxs = randperm(size(new_data, 1));
        dim = size(new_data)[1];
